...
function CheckoutContent() {
  ...
  const [coupon,        setCoupon]        = useState(null);  // { couponId, name, discountAmount, finalAmount }
  ...
  // For daily: strip time before diffing so "08:00 → 20:00 over 2 days" = 2, not 3
  ...
  // Driver charge: per-day rate × booking days (0 if driver included or no charge set)
  ...
  // couponBase: what the coupon discount is calculated against
  // For per-km → advance + driver charge; for regular → subtotal + add-ons + driver charge
  ...
  // Build addons array for booking
                          ...
                          {qty > 0 ? (
                            <>
                              <button type="button" onClick={() => changeQty(addon._id, -1)}
                                className="w-7 h-7 rounded-full border border-gray-200 flex items-center justify-center text-gray-600 hover:bg-gray-50 font-bold text-base transition-colors">
                                −
                              </button>
                              <span className="text-sm font-black text-gray-900 w-4 text-center">{qty}</span>
                              <button type="button" onClick={() => changeQty(addon._id, 1)}
                                className="w-7 h-7 rounded-full bg-[#84cc16] hover:bg-lime-400 flex items-center justify-center text-gray-900 font-bold text-base transition-colors">
                                +
                              </button>
                            </>
                          ) : (
