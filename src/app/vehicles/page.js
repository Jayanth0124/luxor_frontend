...
function VehiclesContent() {
  ...
  const [nearMe,  setNearMe]  = useState(false);  // true when using browser geolocation
  ...
  const { data, isLoading } = useQuery({
    queryKey: ['public-vehicles', search, state, city, nearMe, userLat, userLng, page],
    queryFn: () => getVehicles({
  ...
  return (
    <div className="bg-[#f8fafc] min-h-screen">
      ...
      {/* ── Page hero banner ── */}
      <div className="relative pt-16" style={{ background: 'linear-gradient(135deg, #0a0f1a 0%, #0f1f0a 60%, #1a2e00 100%)' }}>
        {/* Decorative glow — overflow clipped independently so dropdowns aren't affected */}
        ...
        <div className="max-w-screen-xl mx-auto px-4 sm:px-6 md:px-12 xl:px-20 pt-10 pb-12">
          <p className="text-[#84cc16] text-xs font-black uppercase tracking-[0.22em] mb-3">Premium Fleet</p>
          <h1 className="text-white font-black leading-none mb-3" style={{ fontSize: 'clamp(2rem, 4vw, 3rem)', letterSpacing: '-0.02em' }}>
            Browse All Vehicles
          </h1>
          <p className="text-white/45 text-sm mb-8 max-w-lg">Explore our handpicked fleet of luxury self-drive and chauffeur vehicles across India.</p>

          {/* Filters inline in banner */}
      ...
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 md:px-12 xl:px-20 py-10">
        ...
        {!isLoading && (
          <p className="text-xs text-gray-400 mb-6">{total} vehicle{total !== 1 ? 's' : ''} found</p>
        )}
        ...
        {isLoading ? (
          ...
          <div className="text-center py-24">
            <p className="text-gray-400 text-sm mb-3">No vehicles found for your search.</p>
